...
Future<User> login(User user) async {
  ...
  print('User: ${json.encode(user.toMap())}');
  if (response.statusCode == 200) {
    setCurrentUser(response.body);
    currentUser.value = User.fromJSON(json.decode(response.body)['data']);
  } else {
    print(
        "LOGIN ERROR:${CustomTrace(StackTrace.current, message: response.body).toString()}");
...
Future<User> register(User user) async {
  final String url =
      '${GlobalConfiguration().getValue('api_base_url')}register1';
  final client = new http.Client();
  final response = await client.post(
    Uri.parse(url),
    headers: {HttpHeaders.contentTypeHeader: 'application/json'},
  ...
  print("REGISTER RESPONSE BODY : ${jsonDecode(response.body)}");
  if (response.statusCode == 200) {
    setCurrentUser(response.body);
    currentUser.value = User.fromJSON(json.decode(response.body)['data']);
  } else {
...
Future<User> update(User user) async {
  ...
  print('USER UPDATE :${response.body}');
  setCurrentUser(response.body);
  currentUser.value = User.fromJSON(json.decode(response.body)['data']);
  return currentUser.value;
}
