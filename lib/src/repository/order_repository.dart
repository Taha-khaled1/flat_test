...
Future<Order> addOrder(Order order, Payment payment) async {
  ...
  final client = new http.Client();
  order.isPoint=usePoints;//TO GET POINT
  Map <String ,dynamic>params = order.toMap();
  if(order.deliveryAddress.longitude==null||order.deliveryAddress.latitude==null)
    {

    }
  params.addAll(_creditCard.toMap());
  params.addAll({'isPoints':usePoints});
  ...
  print('url = $url');
  print('order = ${json.encode(params)}');
