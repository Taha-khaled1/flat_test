...
import 'package:turkey_app/src/helpers/custom_trace.dart';
import 'package:turkey_app/src/helpers/maps_util.dart';
import 'package:turkey_app/src/models/settings.dart';
import 'package:turkey_app/src/pages/delivery_pickup.dart';
...
Future<Settings> getSettings() async {
  Uri url = Helper.getUri('api/settings');
  http.Response response = await http.get(url);
  settings = Settings.fromJson(jsonDecode(response.body));
  regularPaymentToken = settings.data.myfatoorahKey;
  if (settings.data.enable_myfatoorah == '0') {
    isPaypal = false;
  } else {
    isPaypal = true;
  }
  print('isPayPal : $isPaypal');
  return settings;
}
